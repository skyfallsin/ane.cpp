...
def _wait_for_server(port: int, timeout: int = SERVER_STARTUP_TIMEOUT) -> bool:
    """Poll server until it can handle completion requests.
    
    Two-phase readiness check:
    1. Wait for /v1/models to return 200 (server process is up)
    2. Send a tiny completion probe to verify model is loaded and accepting requests
       (llama-server returns 503 while model is still loading even though /v1/models works)
    """
    ...
    # Phase 2: probe with a tiny completion to confirm model is loaded
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if e.code == 503:
                pass  # model still loading, keep waiting
            else:
                return True  # non-503 means model is loaded (could be 400 etc)
        except (urllib.error.URLError, ConnectionRefusedError, OSError, http.client.RemoteDisconnected):
            pass
        if attempt % 10 == 0:
            elapsed = time.monotonic() - (deadline - timeout)
...
def _fill_context(worker_id: int, port: int, target_tokens: int,
                  print_lock: threading.Lock) -> list:
    ...
    paragraphs = [
        ...
        "foundations that would eventually lead to modern deep learning systems.",
        "Quantum mechanics describes nature at the smallest scales of energy levels of "
        "atoms and subatomic particles. Max Planck originated quantum theory in 1900 when "
        "he proposed that energy is radiated and absorbed in discrete quanta.",
        ...
        "to the rest of Europe by the 16th century influencing literature, philosophy, "
        "art, music, politics, science, religion, and other aspects of inquiry.",
        "Oceanic currents are continuous directed movements of seawater generated by "
        "forces acting upon the mean flow such as wind, Coriolis effect, temperature "
        "and salinity differences, and tides caused by gravitational pull of the moon.",
        "The theory of general relativity published by Albert Einstein in 1915 remains "
        "the current description of gravitation in modern physics. It generalises special "
        "relativity and refines Newton\'s law of universal gravitation providing a unified "
        "description of gravity as a geometric property of space-time.",
        ...
        "released to fuel the organism\'s metabolic activities.",
        "The Industrial Revolution was the transition to new manufacturing processes in "
        "Great Britain Europe and the United States in the period from about 1760 to 1840. "
    ...
    chars_needed = (target_tokens - 200) * 5
    parts = []
    total_chars = 0
    i = 0
    ...
    # Send as system + user message, ask for short reply
    conversation = [
        {"role": "system", "content": filler},
        {"role": "user", "content": "Summarize the above in one sentence."},
    ]
    # Generous timeout: each fill takes ~250s at 32K, and fills serialize in queue.
    # Worker N may wait N*250s before the server starts processing its request.
    fill_timeout = 600 + worker_id * 300
    tr = _send_turn(port, conversation, max_tokens=20, timeout=fill_timeout)
    with print_lock:
        if tr.error:
            print(f"  W{worker_id}: context fill FAILED: {tr.error}")
        else:
            print(f"  W{worker_id}: context filled to {tr.prompt_tokens}pt ({tr.wall_s:.1f}s)")

    if not tr.error and tr.prompt:
...
def _run_conversation_worker(worker_id: int, port: int, deadline: float,
                             ...
                             prefilled_conversation: list = None):
    """Worker thread: runs one conversation loop until deadline."""
    conversation = prefilled_conversation if prefilled_conversation else []
    turn_num = 0
    ...
    while time.monotonic() < deadline:
        ...
        turn_num += 1
        per_turn_timeout = min(180, max(30, int(deadline - time.monotonic())))
        ...
        if not tr.error:
            ...
            if tr.prompt:
                conversation.append({"role": "assistant", "content": tr.prompt})
        elif "400" in str(tr.error):
            # Context overflow — trim conversation to prevent infinite 400 loops.
            # Keep system message (if any) + last few turns.
            ...
            conversation = sys_msgs + non_sys[-4:] if non_sys else sys_msgs
...
def run_sustained(config_key: str, duration_min: float = 5.0,
    ...
    # --- Pre-fill context (before timer and powermetrics) ---
    print_lock = threading.Lock()
    prefilled = [None] * n_parallel
    if context_length > 4096 and n_parallel > 1:
        # Fill each session's context to near capacity before measuring
        fill_target = context_length - 1024  # leave room for ongoing conversation
        print(f"  📝 Pre-filling {n_parallel} sessions to ~{fill_target} tokens...")

        def _fill_worker(w):
            prefilled[w] = _fill_context(w, port, fill_target, print_lock)

        fill_threads = []
        for w in range(n_parallel):
            t = threading.Thread(target=_fill_worker, args=(w,), daemon=True)
            fill_threads.append(t)
            t.start()
            time.sleep(0.2)  # stagger to avoid prefill queue pileup
        for t in fill_threads:
            t.join(timeout=2400)  # 6 sessions × 250s/fill serialized = 1500s + margin
        print(f"  \u2705 Context pre-fill complete")


    # Brief pause to let thermals settle after heavy prefill
    if any(p is not None for p in prefilled):
        print(f"  \u23f8\ufe0f  Settling 5s before timed measurement...")
        time.sleep(5)

    # --- Start powermetrics (after pre-fill for clean measurement) ---
    pm_out = tempfile.NamedTemporaryFile(delete=False, suffix=".plist")
    pm_out.close()
    pm_proc = subprocess.Popen(
        POWERMETRICS_CMD,
    ...
    if n_parallel == 1:
        # Single-threaded path — also prefill if needed
        single_prefill = None
        if context_length > 4096 and not prefilled[0]:
            fill_target = context_length - 1024
            print(f"  📝 Pre-filling context to ~{fill_target} tokens...")
            single_prefill = _fill_context(0, port, fill_target, print_lock)
            wall_start = time.monotonic()  # restart timer after prefill
            deadline = wall_start + duration_s
            print()
...
def _kill_port(port: int):
    """Kill any process listening on the given port (cleanup from prior runs)."""
    try:
    ...
    if proc and proc.poll() is None:
        try:
            ...
            try:
                ...
                pass
        # Belt-and-suspenders: also kill individual process if group kill missed it
